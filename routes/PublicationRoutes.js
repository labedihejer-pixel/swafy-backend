...
// ===============================
// 📋 GET /publications/public - Public publications (no auth)
// ===============================
...
router.post("/", verifyToken, upload.array("files", 10), async (req, res) => {
  try {
    ...
    const { titre_publication, contenu_publication, type_publication = "texte", debat_side } = req.body;
    ...
    const [result] = await db.query(
      `INSERT INTO publications 
       ...
       VALUES (?, ?, ?, ?, NOW(), 'publie')`,
      [userId, titre_publication.trim(), contenu_publication?.trim() || "", finalType]
    );
    ...
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        ...
        console.log(`📎 Media added: ${urlMedia}`);
      }
    }
